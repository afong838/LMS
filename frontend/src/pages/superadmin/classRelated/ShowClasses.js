...
const ShowClasses = () => {
  ...
  const SclassButtonHaver = ({ row }) => {
    const actions = [
      { icon: <PostAddIcon />, name: 'Add Subjects', action: () => navigate("/SuperAdmin/addsubject/" + row.id) },
      { icon: <PersonAddAlt1Icon />, name: 'Add Student', action: () => navigate("/SuperAdmin/class/addstudents/" + row.id) },
    ];
    return (
      <ButtonContainer>
        ...
        <>
          {getresponse ?
            <Box sx={{ display: 'flex', justifyContent: 'flex-end', marginTop: '16px' }}>
              <GreenButton variant="contained" onClick={() => navigate("/SuperAdmin/addclass")}>
                Add Class
              </GreenButton>
            </Box>
            :
